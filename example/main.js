...
var steem_ops = {"ref_block_num":31525,"ref_block_prefix":1680218073,chainId: "optional","operations":[["vote",{"voter":"voter1","author":"author1","permlink":"permlink-for-post","weight":2500}],
                ["vote",{"voter":"voter2","author":"author2","permlink":"another-permlink-for-post","weight":2500}]]};
...
var sb_demos = "var steem_ops = " + JSON.stringify(steem_ops, null, 2) + 
               ...
               "<br/><br/>1. if successful, result will include signature in hex, \n   and expiration in response.data.message.expiration<br/>" +
               "<br/>2. experimental, if steem_ops.url is included w/o ref_block data, \n   tx will be signed and broadcast and result will include tx_id and block_num<br/><br/>" +
               "<br/>var eos_ops = " + JSON.stringify(eos_ops, null, 2) + 
               "<br/>whalevault.requestSignBuffer('appid', 'eos:eosuser', eos_ops, 'Active', 'testTx', 'tx', \n           function (response) { console.log(response); })" +
               "<br/><br/>1. if expiration, ref_block_num, and ref_block_prefix are present,\n   a JsSignatureProvider-style EOS signature will be returned<br/>" +
               "<br/>2. tx will be signed and broadcast and result will include tx_id and block_num<br/>" +
