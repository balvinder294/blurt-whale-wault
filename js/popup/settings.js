    ...
    chrome.storage.local.get(['no_confirm'], function(items) {
        try {
            ...
            for (let obj in pref[name]) {
                $("#pref").append("<h4>site: <font color=yellow>" + obj + "</font></h4>");
                var display_names = {
                    'broadcast': 'Broadcast',
                ...
                for (let sub in pref[name][obj]) {
                    site_container.append("<div><div class='pref_name'>" + sub + "</div><img id='" + name + "," + obj + "," + sub + "' class='deletePref' src='../images/delete.png'/></div>");
                }
